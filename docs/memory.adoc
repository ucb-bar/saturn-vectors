...
Unit-strided (segmented and non-segmented) accesses do not execute the inner loop, and they iterate the outer loop by the number of elements requested by the next request.
...
Strided and indexed non-segmented accesses do not execute the inner loop, and they iterate the outer loop by a single element per cycle.
...
Each segment buffer is implemented as a double-buffered 2D array of flip-flops.
