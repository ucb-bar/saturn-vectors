 ...
 * Short-vector machines can still reduce IPC requirements on applications with long vector lengths by leveraging the register grouping capabilities of modern vector ISAs
...
While VLIW instruction encodings effectively provide high operation throughput to the datapath, leveraging them requires precisely optimized software-pipelined loops.
