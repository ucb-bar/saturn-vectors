...
class OuterProductSequencer(params : OPUParameters, exu_insns: Seq[VectorInstruction])(implicit p: Parameters) extends Sequencer[ExecuteMicroOp]()(p) {
  ...
  val opu_array_nrows = dLen/params.A_width         // Dimension of PE array 
  ...
  io.opu_cntrl.row_en := (0.U(opu_array_nrows.W)).asBools
  io.opu_cntrl.read_en := (0.U(opu_array_nrows.W)).asBools
  ...
  switch(ctrl_st) {
    ...
    is(MVOUT0){
      ...
      io.opu_cntrl.row_en   := UIntToOH(mvout_mrow(log2Ceil(opu_array_nrows)-1, 0)).asBools       // Indicate that row being read
    ...
    //     io.opu_cntrl.read_en  := UIntToOH(mvout_mrow(log2Ceil(opu_array_nrows)-1, 0)).asBools       // Indicate that row being read
    //   } .otherwise {
    //     regidx := regidx
    ...
    //   // regidx := Mux(mvout_cnt < varch_ratio.U, Cat(mvout_mreg, mvout_mrow + mvout_cnt), regidx)   // SLOPPY BUT WHATEVER
    //   // io.opu_cntrl.read_en  := Mux(mvout_cnt < varch_ratio.U, UIntToOH(mvout_mrow(log2Ceil(opu_array_nrows)-1, 0)).asBools       // Indicate that row being read
    ...
    is(OPROD){
      ...
      when (io.rvs1.fire && io.rvs2.fire) {
        ...
        io.opu_cntrl.row_en := ((-1).S(opu_array_nrows.W)).asBools
      } .otherwise {
        op_egidx := op_egidx
        io.opu_cntrl.row_en := (0.U(opu_array_nrows.W)).asBools
      }
