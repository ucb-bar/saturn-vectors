...
class SegmentedMultiplyPipe(depth: Int)(implicit p: Parameters) extends PipelinedFunctionalUnit(depth)(p) {
  ...
  val in_vs1 = io.pipe(depth-1).bits.rvs1_data
  val in_vs2 = io.pipe(depth-1).bits.rvs2_data
  val in_vd  = io.pipe(depth-1).bits.rvd_data
  ...
  for (i <- 0 until (dLenB >> 3)) {
    ...
    multipliers(i).io.eew        := io.pipe(depth-1).bits.rvs1_eew
  ...
  // //////////////////////////////////////////////
  // val in_eew_pipe = io.pipe(0).bits.rvs1_eew
  // val out_eew_pipe = io.pipe(0).bits.vd_eew
  // val ctrl_wmul = out_eew_pipe > in_eew_pipe
  // val ctrl_smul = io.pipe(0).bits.isOpi
  // val ctrl_pipe = new VectorDecoder(io.pipe(0).bits.funct3, io.pipe(0).bits.funct6, 0.U, 0.U, supported_insns, Seq(
  //   MULHi, MULSwapVdV2, MULAccumulate, MULSub))
  // val in_vs2_pipe = io.pipe(0).bits.rvs2_data
  // val in_vd_pipe  = io.pipe(0).bits.rvd_data
  ...
  val half_sel = (io.pipe(depth-1).bits.eidx >> (dLenOffBits.U - out_eew))(0)
  ...
  val smul_prod = VecInit.tabulate(4)({sew =>
    if (sew == 3 && dLenB == 8) { mul_out.asSInt } else {
      mul_out.asTypeOf(Vec(dLenB >> sew, SInt((16 << sew).W)))(io.pipe(depth-1).bits.eidx(log2Ceil(dLenB)-1-sew,0))
    }
  })(out_eew).asSInt

  val rounding_incr = VecInit.tabulate(4)({ sew => RoundingIncrement(io.pipe(depth-1).bits.vxrm, smul_prod((8 << sew)-1,0)) })(out_eew)
  ...
  // val out = Pipe(io.pipe(depth-1).valid, out, depth-3).bits
  // val vxsat = Pipe(io.pipe(depth-1).valid, smul_sat && ctrl_smul, depth-3).bits
  val vxsat = smul_sat && ctrl_smul
  ...
  io.set_vxsat := io.pipe(depth-1).valid && vxsat
