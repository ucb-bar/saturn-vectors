...
class VFDivSqrt(implicit p: Parameters) extends IterativeFunctionalUnit()(p) with HasFPUParameters {
  ...
  when (io.iss.op.rvs1_eew === 3.U) {
    ...
    val widen = Seq(FType.S.recode(rvs2_bits(31,0)), FType.S.recode(rvs1_bits(31,0))).zip(
    ...
    divSqrt.io.b := Mux(ctrl_swap12, widen(0).io.out, widen(0).io.out)
  }
  ...
  val divSqrt_out = Mux(op.vd_eew === 3.U, FType.D.ieee(divSqrt.io.out), FType.S.ieee(narrow.io.out))
