...
class ExecutionUnit(fus: Seq[FunctionalUnit])(implicit val p: Parameters) extends HasCoreParameters with HasVectorParams {
  ...
  val hazards = Wire(Vec(nHazards, Valid(new PipeHazard(pipe_depth))))
  val vrf_write_hazards = Wire(Vec(pipe_depth, Valid(new PipeHazard(pipe_depth))))
  val issue_pipe_hazard = Wire(Valid(new PipeHazard(pipe_depth)))
  val inflight_hazard_stall = WireInit(false.B)
  val issue_hazard_stall = WireInit(false.B)
  val busy = Wire(Bool())
  ...
  fus.foreach { fu =>
    ...
    fu.io.iss.valid := iss.valid && !pipe_stall && !inflight_hazard_stall && !issue_hazard_stall
  }

  val pipe_write_hazard = WireInit(false.B)
  val readies = fus.map(_.io.iss.ready)
  iss.ready := readies.orR && !pipe_write_hazard && !pipe_stall && !inflight_hazard_stall && !issue_hazard_stall
  when (iss.valid) { assert(PopCount(readies) <= 1.U) }

  issue_pipe_hazard.valid         := readies.orR && iss.valid && !inflight_hazard_stall
  issue_pipe_hazard.bits.vat      := iss.bits.vat
  issue_pipe_hazard.bits.latency  := Mux1H(pipe_fus.map(_.io.iss.ready), pipe_fus.map(_.depth.U)) 
  issue_pipe_hazard.bits.eg       := iss.bits.wvd_eg
  ...
  if (pipe_fus.size > 0) {
    ...
    for (i <- 0 until pipe_depth) {
      ...
      hazards(i).bits.latency := DontCare
      vrf_write_hazards(i).valid       := pipe_valids(i)
      vrf_write_hazards(i).bits.vat    := pipe_bits(i).vat
      vrf_write_hazards(i).bits.eg     := pipe_bits(i).wvd_eg
      when (pipe_latencies(i) === 0.U) { // hack to deal with compress unit
        vrf_write_hazards(i).bits.eg   := Mux1H(pipe_sels(i), pipe_fus.map(_.io.write.bits.eg))
      }
      vrf_write_hazards(i).bits.latency := pipe_latencies(i)
    }
  }
