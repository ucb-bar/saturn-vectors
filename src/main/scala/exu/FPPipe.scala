...
class FPPipe(implicit p: Parameters) extends PipelinedFunctionalUnit(3, true)(p) with HasFPUParameters {
  ...
  lazy val ctrl_table = Seq(
    (OPFFunct6.vfadd,     Seq(Y,N,N,N,N)),
    (OPFFunct6.vfsub,     Seq(Y,N,N,Y,N)),
    (OPFFunct6.vfrsub,    Seq(Y,N,N,N,Y)),
    (OPFFunct6.vfmul,     Seq(N,Y,N,N,N)),
    (OPFFunct6.vfmacc,    Seq(Y,Y,N,N,N)),
    (OPFFunct6.vfnmacc,   Seq(Y,Y,N,Y,Y)),
    (OPFFunct6.vfmsac,    Seq(Y,Y,N,Y,N)),
    (OPFFunct6.vfnmsac,   Seq(Y,Y,N,N,Y)),
    (OPFFunct6.vfmadd,    Seq(Y,Y,Y,N,N)),
    (OPFFunct6.vfnmadd,   Seq(Y,Y,Y,Y,Y)),
    (OPFFunct6.vfmsub,    Seq(Y,Y,Y,Y,N)),
    (OPFFunct6.vfnmsub,   Seq(Y,Y,Y,N,Y)),
  )
  override def accepts(f3: UInt, f6: UInt): Bool = f3.isOneOf(OPFVV, OPFVF)
  val ctrl_add :: ctrl_fma :: ctrl_swap23 :: ctrl_fmaCmd0 :: ctrl_fmaCmd1 :: Nil = VecDecode.applyBools(
  ...
  val fma_pipes = Seq.fill(fmaCount)(Module(new TandemFMAPipe(depth))).zipWithIndex.map { case(fma_pipe, i) =>
    val rs1_bits = Mux(is_opfvf, scalar_operand_bits, vec_rvs1(i))

    when (ctrl_fma && ctrl_add) {
      fma_pipe.io.a := rs1_bits
      when (ctrl_swap23) {
        fma_pipe.io.b := vec_rvd(i)
        fma_pipe.io.c := vec_rvs2(i)
      } .otherwise {
        fma_pipe.io.b := vec_rvs2(i)
