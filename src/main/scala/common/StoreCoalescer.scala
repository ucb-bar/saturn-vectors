...
class StoreCoalescer(implicit p: Parameters) extends CoreModule()(p) with HasVectorParams {
  ...
  when (io.saq.bits.prestart || io.saq.bits.masked) {
    ...
    io.req.valid := io.saq.valid && io.stdata.valid
    io.saq.ready := io.stdata.valid && io.req.ready
    io.stdata.ready := io.saq.valid && io.req.ready
  } .otherwise {
    io.req.valid := io.saq.valid && (io.stdata.valid || io.saq.bits.tail)
    io.saq.ready := io.req.ready && (io.stdata.valid || io.saq.bits.tail)
    io.stdata.ready := io.req.ready && io.saq.valid && !io.saq.bits.tail
  }
