...
class SegmentedIntegerMultiplier(depth: Int)(implicit p: Parameters) extends CoreModule()(p) with HasVectorParams {
    ...
    for (vsew <- 0 until 4) {
        ...
        // println("blockValue", blockValue.toBinaryString)
        ...
        for (i <- 0 until numBlocks) {
            for (j <- 0 until lenBlocks) { //lenBlocks = element witdth (in bytes)
                ...
                // println("(blockValue << (lenBlocks*i)", (blockValue << (lenBlocks*i)).toBinaryString)
                // println("validPP 0 = ", (blockValue << (lenBlocks*i)).U(0))
                // println("validPP 1 = ", (blockValue << (lenBlocks*i)).U(1))
                // println("validPP 2 = ", (blockValue << (lenBlocks*i)).U(2))
                // println("validPP 5 = ", (blockValue << (lenBlocks*i)).U(5))
                // println("validPP 6 = ", (blockValue << (lenBlocks*i)).U(6))
                // println("validPP 7 = ", (blockValue << (lenBlocks*i)).U(7))
            }
        }
    }
    ...
    for (vsew <- 0 until 4) {
        ...
        val vSumPProdsU = SumPProdsU.asTypeOf(Vec(nElems, UInt((2*eew).W)))
