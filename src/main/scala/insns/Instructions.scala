...
object ADC       extends OPIInstruction    { val props = Seq(F6(OPIFunct6.adc)     , DoSub.N, Averaging.N, CarryIn.Y, AlwaysCarryIn.N, SetsWMask.N) }
object MADC      extends OPIInstruction    { val props = Seq(F6(OPIFunct6.madc)    , DoSub.N, Averaging.N, CarryIn.Y, AlwaysCarryIn.Y, SetsWMask.N) }
object SBC       extends OPIInstruction    { val props = Seq(F6(OPIFunct6.sbc)     , DoSub.Y, Averaging.N, CarryIn.Y, AlwaysCarryIn.N, SetsWMask.N, WritesAsMask.Y) }
object MSBC      extends OPIInstruction    { val props = Seq(F6(OPIFunct6.msbc)    , DoSub.Y, Averaging.N, CarryIn.Y, AlwaysCarryIn.Y, SetsWMask.N, WritesAsMask.Y) }
...
object FCVT_SGL  extends VectorInstruction { val props = Seq(F6(OPFFunct6.funary0), F3(VectorConsts.OPFVV), RS1(BitPat("b00???"))) }
...
object SLIDEUP   extends OPIInstruction    { val props = Seq(F6(OPIFunct6.slideup), UsesIndexMaskSeq.Y, ReadsVS1.N, ReadsVS2.N) }
