...
void mm_opu(uint8_t* A, uint8_t* B, uint32_t* C, size_t M, size_t N, size_t K, size_t altfmt) {
    ...
    while (i < M) {
        ...
        while (j < N) {
            // Clear the m1 tile
            size_t cols;
            asm volatile("vsetvli %[vl], %[avl], e32, m4, ta, ma" : [vl]"=r"(cols) : [avl]"r"(N-j));
            asm volatile("vmv.v.i v0, 0x0");
            OPMVINBCAST(m1, v0);

            for (size_t k = 0; k < K; k++) {
                asm volatile("vsetvli x0, %[avl], e8, m1, ta, ma" : : [avl]"r"(rows));
                asm volatile("vle8.v v0, (%0)" : : "r"(&A[M*k+i]));
                if (altfmt == 0) {
                    asm volatile("vsetvli x0, %[avl], e8, m1, ta, ma" : : [avl]"r"(cols));
                    // VSETVLI_ALTFMT_X0(N-j, SEW_E8, LMUL_M1, 0);
                }
                else {
                    VSETVLI_ALTFMT_X0(cols, SEW_E8, LMUL_M1, 1);
                }
