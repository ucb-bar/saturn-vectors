...
void i32_load_c(int32_t* c) {
  asm volatile("vle32.v v0, (%0)" : : "r"(c));
  OPMVINBCAST(m0, v0); // move v0 into row r of m0
  OPMVINBCAST(m2, v0); // move v0 into row r of m2
}
...
void i32_store_c(int32_t* c, size_t ml, size_t N) {
    for (size_t r = 0; r < ml; r++) {
      ...
      VMV_VR(v8, r, m2); // move row r of m2 into v2
...
void i32_set_c_opu(int32_t* c_bias, int32_t* c_out, size_t N, size_t M) {
  size_t mlmax, vl;
  asm volatile("vsetvli %0, zero, e32, m8, ta, ma" : "=r"(mlmax));
  size_t i = 0;
  while (i < M) {
