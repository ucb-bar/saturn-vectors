...
void i8_mm_bme_2x2(int32_t* c_bias, int32_t* c_out, int8_t* at, int8_t* b, size_t M, size_t N, size_t K, size_t m_start, size_t m_end) {
  size_t mlmax;
  asm volatile("vsetvli %0, zero, e8, m1, ta, ma" : "=r"(mlmax));
  size_t vl;
  size_t i = m_start;
  while (i + 2*mlmax <= m_end) {
    size_t j = 0;
  ...
  while (i < m_end) {
    size_t ml = (m_end - i) > mlmax ? mlmax : m_end - i;
    for (size_t j = 0; j < N;) {
      asm volatile("vsetvli %0, %1, e32, m4, ta, ma" : "=r"(vl) : "r"(N - j));
