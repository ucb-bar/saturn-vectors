...
public:
  ...
  size_t kernel(T* out, size_t n) {
    ...
    size_t consumed = (consumed1 + consumed2 + consumed3 + consumed4) / sizeof(T);
    work_queue.front().first += consumed;
    work_queue.front().second -= consumed;
...
public:
  ...
  size_t kernel(uint32_t* in, uint32_t* out, size_t n) {
    size_t avl = n;
    ...
    load2 = __riscv_vmul_vx_u32m8(load2, scale, consumed2);
    ...
    load3 = __riscv_vmul_vx_u32m8(load3, scale, consumed3);
    ...
    load4 = __riscv_vmul_vx_u32m8(load4, scale, consumed4);
    __riscv_vse32_v_u32m8(out, load4, consumed4);
    avl -= consumed4;
    in += consumed4;
    out += consumed4;

    return consumed1 + consumed2 + consumed3 + consumed4;
  }
private:
  uint32_t scale;
};
